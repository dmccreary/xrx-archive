...
let $id := doc('/db/dma/apps/comments/edit/next-id.xml')//next-id/text()
let $title := concat('Save Comment ', $id)
...
let $update-id := update replace doc(concat($data-collection, '/', $file))//feedback/id with <id>{$id}</id>
let $doc := collection($data-collection)//feedback[./id/text() = $id]
...
let $new-next-id := update value doc($next-id-file-path)//next-id with ($id + 1)
...
let $content := 
    <div>
        <p>Comment {$original-id} has been saved.</p>
