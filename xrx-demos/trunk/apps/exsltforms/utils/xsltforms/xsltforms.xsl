...
-->
<xsl:stylesheet xmlns:txs="http://www.agencexml.com/txs" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:default="http://www.w3.org/1999/xhtml" xmlns:exslt="http://exslt.org/common" xmlns:ev="http://www.w3.org/2001/xml-events" xmlns:xforms="http://www.w3.org/2002/xforms" xmlns:xhtml="http://www.w3.org/1999/xhtml" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:msxsl="urn:schemas-microsoft-com:xslt" xmlns:ajx="http://www.ajaxforms.net/2006/ajx" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="1.0" exclude-result-prefixes="xhtml xforms ev exslt msxsl">
    ...
    <xsl:include href="../exsltforms/exsltforms.xsl"/>	
<!--
...
-->
    ...
    <xsl:variable name="configdoc" select="document(concat($xsltforms_home,'config.xsl'))/xsl:stylesheet/xsl:template[@name='config']"/>
    <xsl:variable name="config0">
        <xsl:choose>
            <xsl:when test="$configdoc/properties">
                <xsl:copy-of select="$configdoc"/>
            </xsl:when>
            <xsl:otherwise>
                <config>
                    <options>
    ...
    <xsl:template xmlns:xalan="http://xml.apache.org/xalan" name="html">
        ...
        <html xmlns="http://www.w3.org/1999/xhtml">
            <xsl:copy-of select="@*"/>
            <xsl:comment>HTML elements and Javascript instructions generated by XSLTForms r494 - Copyright (C) 2008-2011 &lt;agenceXML&gt; - Alain COUTHURES - http://www.agencexml.com</xsl:comment>
            ...
            <body>
                ...
                <div id="xsltforms_console">&#160;<xsl:text/>
                </div>
                <div id="statusPanel">
                    <xsl:value-of select="$config/properties/status"/>&#160;<xsl:text/>
                </div>
            </body>
        </html>
    </xsl:template>
...
-->
    <xsl:template match="xhtml:thead[xforms:repeat] | thead[xforms:repeat] | xhtml:tbody[xforms:repeat] | tbody[xforms:repeat] | xhtml:tfoot[xforms:repeat] | tfoot[xforms:repeat] | xhtml:tr[xforms:repeat] | tr[xforms:repeat]">
        <xsl:apply-templates select="xforms:repeat"/>
    </xsl:template>
    <xsl:template match="xhtml:table[xforms:repeat] | table[xforms:repeat]">
        <table>
            <xsl:apply-templates select="xforms:repeat"/>
        </table>
    </xsl:template>
...
-->
    <xsl:template match="xforms:dispatch" mode="script" priority="1">
        ...
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@targetid"/>
        </xsl:call-template>
        <xsl:text>,</xsl:text>
        <xsl:call-template name="toScriptParam">
            <xsl:with-param name="p" select="@if"/>
        ...
        <xsl:if test="@delay | xforms:delay">
            ...
            <xsl:choose>
                <xsl:when test="xforms:delay/@value">
                    ...
                    <xsl:call-template name="toXPathExpr">
                        <xsl:with-param name="p" select="xforms:delay/@value"/>
                    </xsl:call-template>
                    <xsl:text>")</xsl:text>
                </xsl:when>
                <xsl:when test="xforms:delay">
                    <xsl:call-template name="toScriptParam">
                        <xsl:with-param name="p" select="xforms:delay"/>
                    </xsl:call-template>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:call-template name="toScriptParam">
                        <xsl:with-param name="p" select="@delay"/>
                    </xsl:call-template>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:if>
...
-->
    <xsl:template match="@*" mode="island" priority="0">
        <xsl:param name="nodeid"/>
        <node id="xf-node-{$nodeid}-{translate(name(),':','|')}" type="attribute" name="{local-name()}" prefix="{substring-before(name(),':')}" ns="{namespace-uri()}">
            <xsl:value-of select="."/>
        </node>
    </xsl:template>
...
-->
    <xsl:template match="*" mode="island" priority="0">
        ...
        <node id="xf-node-{$prefix}-{$nodeid}" type="element" name="{local-name()}" prefix="{substring-before(name(),':')}" ns="{namespace-uri()}">
            <xsl:apply-templates select="@*" mode="island">
                <xsl:with-param name="nodeid" select="concat($prefix,'-',$nodeid)"/>
            </xsl:apply-templates>
            <xsl:apply-templates select="* | text()" mode="island">
...
-->
    <xsl:template match="xforms:instance" mode="island" priority="1">
        <xsl:variable name="instid" select="count(preceding::xforms:instance|ancestor::xforms:instance)"/>
        <instance id="xf-instance-{$instid}">
            <xsl:choose>
                <xsl:when test="@src">
                    <xsl:apply-templates select="document(@src,/)/*" mode="island">
                        <xsl:with-param name="prefix" select="$instid"/>
                    </xsl:apply-templates>
                </xsl:when>
                <xsl:when test="*">
                    <xsl:apply-templates select="*" mode="island">
                ...
                <xsl:when test="@resource">
                    <xsl:apply-templates select="document(@resource,/)/*" mode="island">
                        <xsl:with-param name="prefix" select="$instid"/>
                    </xsl:apply-templates>
                </xsl:when>
            </xsl:choose>
