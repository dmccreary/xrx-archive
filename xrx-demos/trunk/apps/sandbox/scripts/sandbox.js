...
// add keyboard shortcut to enable ctrl+enter to submit (submitted by Frantia¡ek KuÄoera)
...
function doSave(saveAs) {
    if (saveAs || currentResource.isNew()) {
        ...
        dialog.closeEvent.subscribe(function (type, args) {
            ...
            YAHOO.util.Connect.asyncRequest('PUT', '/rest' + args[1] + '/' + args[0], callback, code);
        ...
        YAHOO.util.Connect.setDefaultPostHeader(false);
        YAHOO.util.Connect.initHeader('Content-Type', currentResource.mimeType);
        YAHOO.util.Connect.asyncRequest('PUT', '/rest' + currentResource.path, callback, code);
        YAHOO.util.Connect.setDefaultPostHeader(true);
    }
}
...
function retrieveStored() {
    ...
    var suffix = 'get-examples.xql?id=' + gup(id);
    log('Loading query examples ...');
    
    YAHOO.util.Connect.asyncRequest('GET', suffix, callback);
}
