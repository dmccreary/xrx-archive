...
<div class="content">
     ...
     <ol>{
        ...
        for $hit in collection($data-collection)//Subscription[contains(*, $q)]
        order by ft:score($hit)
        return
           <li><a href="../views/view-item.xq?id={$hit/id/text()}">{$hit//SubscriptionName/text()}</a> {' '} {$hit/SubscriptionDescriptionText/text()}</li>
      }</ol>
    <div id="searchform">
