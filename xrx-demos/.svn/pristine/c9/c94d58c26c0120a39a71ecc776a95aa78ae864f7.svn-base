...
let $item := collection($data-collection)//book[id/text() = $id]
...
<div class="content">
 <table class="span-12">
  ...
  <tbody>
     ...
     <tr><th class="field-label">Publish date:</th><td>{$item/publish-date/text()}</td></tr>
     <tr><th class="field-label">License:</th><td>{$item/license/text()}</td></tr>
     <tr><th class="field-label">List Price:</th><td>{$item/list-price/text()}</td></tr>
   </tbody>
  </table>
