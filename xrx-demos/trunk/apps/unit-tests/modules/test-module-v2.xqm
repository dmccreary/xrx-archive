...
declare function t:run-test($test as element(test), $count as xs:integer) {
  ...
  let $expanded :=
    ...
    let $OK := (
    	(: this would work a lot better as a for loop... :)
        if ($test/error) then
        		let $pass := $expanded instance of element(error) and contains($expanded, $test/error)
        		return 
        			<error
        				pass="{$pass}">
        				{
        ...
        if (exists($test/xpath)) then
        		for $xp in $test/xpath
        		let $pass := t:test(t:xpath($output, $xp))
        		return
        			<xpath pass="{$pass}">
        				{$xp/@desc, $xp[not($pass)]}
        			</xpath>
