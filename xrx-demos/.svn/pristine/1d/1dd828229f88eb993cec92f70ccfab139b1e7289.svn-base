...
var pluginDefinition = {
	...
	init : function( editor ) {
		...
		var editorID = editor.name,
			generalConfigOptionsDoc = eXSLTForms.registry.plugins.teiann[ 'generalConfigOptionsDoc' ],
			annotatorSpecificationsDoc = eXSLTForms.registry.plugins.teiann[ 'annotatorSpecificationsDoc' ],
			pluginLangDoc = eXSLTForms.registry.plugins.teiann[ 'pluginLangDoc' ],
		...
		if (editor.addMenuItem) {
			...
			editor.addMenuItem( 'editEntityItem', {
				label: pluginLangDoc.querySelector( "Annotator[id = 'teiannEditEntityBtn'] > ToolbarButtonTitle" ).textContent,
  				...
  				group: 'teiannGroup',
				icon: CKEDITOR.basePath + 'plugins/tei-ann/config/images/' + annotatorSpecificationsDoc.querySelector( "Annotator[id = 'teiannEditEntityBtn'] > AnnotatorIconName" ).textContent
			});
			editor.addMenuItem( 'removeEntityItem', {
				label: pluginLangDoc.querySelector( "Annotator[id = 'teiannRemoveEntityBtn'] > ToolbarButtonTitle" ).textContent,
  				...
  				group: 'teiannGroup',
				icon: CKEDITOR.basePath + 'plugins/tei-ann/config/images/' + annotatorSpecificationsDoc.querySelector( "Annotator[id = 'teiannRemoveEntityBtn'] > AnnotatorIconName" ).textContent
			});
		}
...
//		overriding the editor's eXSLTForms function for processing its content on update
		eXSLTForms.registry.textarea2rte[ editorID ].processContentOnUpdate = function( editorContent ) {
			var xsltResult = eXSLTForms_xslt( editorContent, CKEDITOR.basePath + 'plugins/tei-ann/core/xsl/tei2tei-ann.xsl', null );//externalVocab2annotatorVocab.xsl
			if ( !Core.isIE )
				{
...
//		overriding the editor's eXSLTForms function for processing its content on save
		eXSLTForms.registry.textarea2rte[ editorID ].processContentOnSave = function( editorContent ) {
			...
			var xsltResult = eXSLTForms_xslt( editorContent, CKEDITOR.basePath + 'plugins/tei-ann/core/xsl/tei-ann2tei.xsl', null );
			if ( !Core.isIE )
				{
				var serializer = new XMLSerializer();
				var xsltResultSerialized = serializer.serializeToString( xsltResult );
...
//		generation of needed buttons with their commands and dialog boxes (if case)
		...
		for ( var i=0, il = annotatorSpecifications.length; i < il; i++ ) {
			...
			editor.ui.addButton( annotatorID,
				...
				command : annotatorID,
				icon: CKEDITOR.basePath + 'plugins/tei-ann/config/images/' + annotatorSpecification.querySelector( "AnnotatorIconName" ).textContent
			});
			...
			var annotatorPossibleParentElementNames = annotatorSpecification.querySelector( "AnnotatorPossibleParentElementNames" ).textContent,
				annotatorClass = annotatorSpecification.getAttribute( 'name' );
			
			//generation of commands and dialog panels
			switch ( annotatorTypeCode ) {
				case "insert":
					var annotatorOptions = {
						'annotatorClass': annotatorClass,
					...
					editor.addCommand( annotatorID, {
						...
						exec : function( editor ) {
							...
							if ( editor.getSelection().getNative() != '' ) {
								...
								} else { editor.insertElement( CKEDITOR.dom.element.createFromHtml( '<span class="' + this.annotatorOptions.annotatorClass + '">&#160;</span>' ) );
							}
						}
					});
				break;
				case "insert-split":
					...
					editor.addCommand( annotatorID, {
						...
						exec : function( editor ) {
							...
							var futureParentElementName = editor._.elementsPath.list[0].getAttribute( 'class' );
							if ( this.annotatorOptions.annotatorPossibleParentElementNames.indexOf( futureParentElementName ) !== -1 ) {
								var elementClass = this.annotatorOptions.annotatorClass;
				...
				break;
				case "selected-wrap":
					var annotatorOptions = { 'element' : 'span', 'attributes' : { 'class' : annotatorClass } };
				...
				case "selected-wrap-server":
					var annotatorOptions = {
						'dialogID' : annotatorID + 'Panel',
						'validatingXSD' : { 'annotatorPossibleParentElementNames' : annotatorPossibleParentElementNames, 'precedingSibling' : {} },
						'dialogHTMLcontent' : { 'element' : 'span', 'attributes' : { 'class' : annotatorClass } },
						'edit' : 'false',
						'currentAnnotator' : ''
					};
