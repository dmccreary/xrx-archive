...
eXSLTForms.utils.baseURI = document.querySelector( "meta[name = 'exsltformsBaseURI']" ).getAttribute( 'content' );
(function(){
...
transform_action.prototype = new XFAbstractAction();
...
transform_action.prototype.run = function(element, ctx)
	...
	var xmlString = Core.saveXML(this.xmlBinding.evaluate(ctx)[0]);

 	var parametersXNode = this.parametersBinding !== null ? this.parametersBinding.evaluate(ctx)[0] : null;

 	var targetString = Core.saveXML(this.targetBinding.evaluate(ctx)[0]);
	...
	XMLEvents.dispatch(xf_model_extensions, "xforms-transform-insert_" + this.idExtElem);
	XMLEvents.dispatch(xf_model_extensions, "xforms-transform-delete_" + this.idExtElem);
	}
