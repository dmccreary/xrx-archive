...
let $docs := collection($root-collection)/*[created-datetime]

(: get a sorted list and only return valid datetimes :)
let $sorted-docs :=
   for $doc in $docs
   let $date-time := $doc/created-datetime/text()
...
let $content := 
    ...
    <p>Sorted by most recent change.</p>
        ...
        <a ref="last-created.xq?start={$start+$perpage}">Next</a>
    </div>
