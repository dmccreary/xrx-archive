...
declare function local:unzip-entry-data($path as xs:string, $data-type as xs:string, $data as item()?, $param as item()*) {
    if ($param[@name eq 'list']/@value eq 'true') then
        ...
        (: this is the string before the "slash" - usually 'word' or 'docProps' :)
...
let $collection := concat(substring-after($style:db-path-to-app, 'xmldb:exist://'), '/unit-tests/data/input')
