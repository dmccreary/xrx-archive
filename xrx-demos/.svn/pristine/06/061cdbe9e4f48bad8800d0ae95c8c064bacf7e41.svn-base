...
<div class="content">
     ...
     <ol>{
        ...
        for $hit in collection($data-collection)//student[contains(*, $q)]
        order by ft:score($hit)
        return
           <li><a href="../views/view-item.xq?id={$hit/id/text()}">{$hit/user-id/text()}</a> {' '} {$hit/first-name/text()}{' '} {$hit/last-name/text()}</li>
      }</ol>
