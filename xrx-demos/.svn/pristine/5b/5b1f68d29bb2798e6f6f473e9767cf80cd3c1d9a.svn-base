...
let $update-id :=  update replace doc(concat($data-collection, '/', $file))/UseCase/id with <id>{$id}</id>

(: this updates the created by, date and time in the document we just saved :)
let $doc := collection($data-collection)/UseCase[./id/text() = $id]
...
let $content := 
<div class="content">
  <h4>User Story {$original-id} has been saved.</h4>
