   ...
   <p><b>Search results for: </b>{$q} <b> In collection: </b>{$data-collection}</p>
     <ol>{
        (: search any column that matchs this string :)
         for $hit in collection($data-collection)//GeneralProject[contains(*, $q)]
        return
           <li><a href="../views/view-item.xq?id={$hit/id/text()}">{$hit/ProjectName/text()}</a>
           {'  '} {$hit/ProjectScopeText/text()}
           </li>
      }</ol>
