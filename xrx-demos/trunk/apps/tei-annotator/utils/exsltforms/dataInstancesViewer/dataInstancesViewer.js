				...
				instancesDivString +=
					'<div id=instancesDiv_' + modelId + '>' +
						'<select id=instancesSelect_' + modelId + ' width="70" onchange="dataViewer_getInstance(this);">' +
							instancesOptionString.replace(/<option /, "<option selected=\"true\" ") +
						'</select>' +
					'</div>';
				}
			}
		dataInstancesViewerDiv.innerHTML =
			'<div id="modelsSelectDiv">' +
				'<select id="modelsSelect" name="modelsSelect" width="70" onchange="dataViewer_getInstancesIDs(this);">' +
					modelsOptionString.replace(/<option /, "<option selected=\"true\" ") +
				'</select>' +
			'</div>' +
			'<div id="instancesDiv">' +
			...
			'<div id="hiddenInstancesSelectDiv">' +
				instancesDivString
			'</div>';
		dataViewer_getInstancesIDs(document.getElementById("modelsSelect"));
...
function dataViewer_getInstancesIDs(object)
	...
	dataViewer_getInstance(document.getElementById("instancesSelect_" + selectedModelId));
	}
...
function dataViewer_getInstance(object)
	...
	var xmlString = Core.saveXML(xmlXNode);
