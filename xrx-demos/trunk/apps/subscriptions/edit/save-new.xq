...
let $title := concat('Save Subscription ', $id)
...
let $update-id := update replace doc(concat($data-collection, '/', $file))/Subscription/id with <id>{$id}</id>

let $doc := collection($data-collection)/Subscription[./id/text() = $id]
