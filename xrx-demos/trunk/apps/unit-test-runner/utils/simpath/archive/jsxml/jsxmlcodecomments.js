...
// Constructor
function REXML(XML) {
	// set the XML property - this never changes
	this.XML = XML;

	// initialize the rootElement property
	// it will be an XML Element object after parsing
	this.rootElement = null;

	// initialize the only member method
	this.parse = REXML_parse;
	// if there was a valid argument begin parsing
	if (this.XML && this.XML != "") this.parse();
}

	function REXML_parse() {
		// matches that tag name $1 and attribute string $2
		var reTag = new RegExp("<([^>/ ]*)([^>]*)>","g");
		// matches tag name $1, attribute string $2, and text $3
		var reTagText = new RegExp("<([^>/ ]*)([^>]*)>([^<]*)","g");
		// holds the element type within the loop
		var strType = "";
		// tag name
		var strTag = "";
		// text contents
		var strText = "";
		// $2 from reTag and reTagText above
		var strAttributes = "";
		// for special elements (pi, cdata, comments)
		// the starting and ending strings must be stored
		var strOpen = "";
		var strClose = "";
		// keeps track of the level within the tree structure
		var iElements = 0;
		// holds the previous element in the loop
		var xmleLastElement = null;
		// another test to make sure the XML property is properly initialized
		if (this.XML.length == 0) return;
		// the string of XML is broken into an array based on the regular expressions
		// each element is stored in arrElementsUnparsed
		var arrElementsUnparsed = this.XML.match(reTag);
		// it is necessary to have the elements combined with their text stored separately
		var arrElementsUnparsedText = this.XML.match(reTagText);
		// loop increment
		var i=0;
		// skip the first element if the tag name is ?xml
...
function RepeatChar(sChar,iNum) {
	for (var i=0,L=""; i<iNum || (return L); i++,L += sChar);
}
