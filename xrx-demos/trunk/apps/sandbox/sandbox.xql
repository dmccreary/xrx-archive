...
declare function sandbox:save-query($query as xs:string, $description as xs:string) as element() {
    ...
    let $root :=
        if ($root0) then
            ...
            xdb:store("/db/dma/apps/sandbox/data", "examples.xml", <example-queries/>, "text/xml")
...
let $query := collection('/db/dma/apps/sandbox/data')/example-queries[id=$id]
let $file-name := util:document-name($query)
return
    <html xmlns="http://www.w3.org/1999/xhtml">
        <head>
            ...
            <script type="text/javascript" src="scripts/sandbox.js"/>
        </head>
        <body class="yui-skin-sam">
            
            <div class="content">
                {style:header()}<br/><br/><br/><br/><br/>
                {style:breadcrumbs(())}{style:web-depth-in-site()}<br/>
                <form name="main" action="execute.xq" type="post">
                    <div id="top-panel">
                        <div id="messages"></div>
                        ...
                        <div id="right-panel">
                            <div id="query-panel">
                                ...
                                <fieldset id="queries">
                                    <label for="saved">XQuery Examples from <a href="../views/view-item.xq?id={$id}">{$file-name}</a></label>
                                    {sandbox-interface:get-select($id)}
                                </fieldset>
                                <textarea id="codeEditor" 
                                    class="codepress xquery linenumbers-on readonly-off span-23" name="qu" 
                                    style="height:175px; width:100%"></textarea>
