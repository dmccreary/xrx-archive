...
let $id := doc('/db/dma/apps/directory/edit/next-id.xml')//next-id/text() 
let $title := concat('Save Employee ', $id)
...
let $update-id := update replace doc(concat($data-collection, '/', $file))//person/id with <id>{$id}</id>

(: this updates the created by, date and time in the document we just saved :)
let $doc := collection($data-collection)//person[./id/text() = $id]
