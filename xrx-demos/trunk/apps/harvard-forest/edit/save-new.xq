...
let $update-id := update value doc(concat($data-collection, '/', $file))/archiveFile/recordID with $id

let $doc := collection($data-collection)/archiveFile[./recordID/text() = $id]
...
let $content := 
    <div>
       ...
       {style:edit-controls($original-id, 'evx')}
    </div>
